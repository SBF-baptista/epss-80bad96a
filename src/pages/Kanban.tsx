...
import { getInstallationOrders } from "@/services/installationOrderService";

const Kanban = () => {
  const { data: orders = [], isLoading, refetch } = useQuery({
    queryKey: ['installation-orders'],
    queryFn: getInstallationOrders,
  });
    ...
    <div className="p-3 md:p-6 bg-gray-50 min-h-full">
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900">Kanban - Gestão de Instalações</h2>
        </div>
