...
const Kanban = () => {
  ...
  const filteredSchedules = schedules.filter((schedule) => {
    ...
    const matchesModel = !filters.model || schedule.vehicle_model?.toLowerCase().includes(filters.model.toLowerCase());

    const matchesConfig =
      !filters.configurationType || schedule.kit?.name?.toLowerCase().includes(filters.configurationType.toLowerCase());
    ...
    <div className="p-4 md:p-6 lg:p-8 bg-muted/30 min-h-full">
      <div className="max-w-7xl mx-auto space-y-5 md:space-y-6">
        ...
        <div className="flex flex-col gap-1">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">Kanban</h2>
          <p className="text-sm md:text-base text-muted-foreground">Esteira de Pedidos</p>
        </div>

        <FilterBar filters={filters} onFiltersChange={setFilters} orders={[]} />
