-- Create a dedicated sequence for automatic order numbers
CREATE SEQUENCE IF NOT EXISTS auto_order_sequence START 1;
...
BEGIN
  ...
  IF NEW.status = 'homologado' AND OLD.status != 'homologado' AND NEW.created_order_id IS NULL THEN
    ...
    IF incoming_vehicle_record IS NOT NULL THEN
      ...
      -- Generate order number using the sequence (thread-safe)
