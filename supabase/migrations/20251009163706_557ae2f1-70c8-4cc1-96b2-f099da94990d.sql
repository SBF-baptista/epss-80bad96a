-- Modificar função para criar agendamento automático no Kanban quando homologado
    ...
    IF default_kit_id IS NOT NULL AND default_technician_id IS NOT NULL THEN
      ...
      IF NOT schedule_exists THEN
        INSERT INTO kit_schedules (
          ...
          'Agendamento criado automaticamente a partir da homologação #' || NEW.id || ' (id_resumo_venda: ' || incoming_vehicle_record.id_resumo_venda || ')'
        );
...
-- Backfill: criar agendamentos para veículos já homologados que não têm agendamento
DO $$
DECLARE
  rec RECORD;
  incoming_vehicle_record RECORD;
  ...
  schedule_exists boolean;
BEGIN
  -- Buscar todos os cards homologados com clientes no Planning
  FOR rec IN 
    SELECT hc.*, c.id as customer_id
    FROM homologation_cards hc
    JOIN incoming_vehicles iv ON iv.id = hc.incoming_vehicle_id
    JOIN customers c ON c.id_resumo_venda = iv.id_resumo_venda
    WHERE hc.status = 'homologado'
      AND c.show_in_planning = true
  LOOP
    -- Buscar dados do incoming_vehicle
    SELECT * INTO incoming_vehicle_record
    FROM incoming_vehicles
    ...
    IF schedule_exists THEN
      RAISE NOTICE 'Agendamento já existe para homologação % (customer %)', rec.id, v_customer_id;
      CONTINUE;
    END IF;
    ...
    -- Buscar técnico padrão
    SELECT id INTO default_technician_id
    FROM technicians
    ORDER BY created_at DESC
    LIMIT 1;

    -- Criar agendamento se temos os dados necessários
    IF default_kit_id IS NOT NULL AND default_technician_id IS NOT NULL THEN
      INSERT INTO kit_schedules (
        kit_id,
        technician_id,
        ...
        'Placa pendente',
        COALESCE(accessories_list, ARRAY[]::text[]),
        'Agendamento criado via backfill automático (homologação #' || rec.id || ', id_resumo_venda: ' || incoming_vehicle_record.id_resumo_venda || ')'
      );

      RAISE NOTICE 'Backfill: agendamento criado para homologação % (customer %)', rec.id, v_customer_id;
    ELSE
      RAISE NOTICE 'Backfill: impossível criar agendamento para %: kit_id=%, technician_id=%', rec.id, default_kit_id, default_technician_id;
    END IF;
  END LOOP;
END $$;