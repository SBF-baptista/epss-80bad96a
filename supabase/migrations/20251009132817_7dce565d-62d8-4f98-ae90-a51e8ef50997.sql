-- Função melhorada para criar kit_schedule ao mudar para 'homologado' de QUALQUER status anterior
...
BEGIN
  ...
  IF NEW.status = 'homologado' AND (OLD IS NULL OR OLD.status != 'homologado') THEN
    ...
    RAISE NOTICE 'Homologation card % status changed to homologado, checking if kit_schedule needed', NEW.id;
    
    -- Check if schedule already exists for this homologation
    SELECT COUNT(*) INTO existing_schedule_count
    FROM kit_schedules
    WHERE notes ILIKE '%' || NEW.id::text || '%';
    
    IF existing_schedule_count > 0 THEN
      RAISE NOTICE 'Kit schedule already exists for homologation %, skipping creation', NEW.id;
      RETURN NEW;
    END IF;
    ...
    -- Only create kit_schedule for Segsale vehicles (have id_resumo_venda)
    IF incoming_vehicle_record IS NOT NULL AND incoming_vehicle_record.id_resumo_venda IS NOT NULL THEN
      
      RAISE NOTICE 'Processing Segsale vehicle: % % (id_resumo_venda: %)', incoming_vehicle_record.brand, incoming_vehicle_record.vehicle, incoming_vehicle_record.id_resumo_venda;
      ...
      IF default_technician_id IS NOT NULL THEN
        ...
        INSERT INTO kit_schedules (
          ...
          'Agendamento automático Segsale #' || incoming_vehicle_record.id_resumo_venda || ' - ' || NEW.brand || ' ' || NEW.model || ' (homologação #' || NEW.id || ')'
        );
        
        RAISE NOTICE 'Kit schedule created from Segsale homologation % for customer % (id_resumo_venda: %)', NEW.id, customer_id, incoming_vehicle_record.id_resumo_venda;
      ELSE
