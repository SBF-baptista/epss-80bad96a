-- Add id_resumo_venda to customers table to track Segsale origin
ALTER TABLE customers ADD COLUMN IF NOT EXISTS id_resumo_venda integer;

-- Clean up incorrect customers (those without proper Segsale linkage)
DELETE FROM kit_schedules WHERE customer_id IN (
  SELECT id FROM customers WHERE document_number LIKE 'NO_CPF_%'
);

DELETE FROM customers WHERE document_number LIKE 'NO_CPF_%';

-- Drop ALL triggers that depend on the function first
DROP TRIGGER IF EXISTS create_kit_schedule_on_homologation ON homologation_cards;
DROP TRIGGER IF EXISTS create_schedule_on_homologation ON homologation_cards;

-- Now we can safely drop the function
DROP FUNCTION IF EXISTS create_kit_schedule_from_homologation();

-- Recreate the function with proper Segsale tracking
...
-- Recreate trigger
CREATE TRIGGER create_kit_schedule_on_homologation
